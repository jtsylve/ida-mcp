...
class TestSpawnLock:
    def test_lock_is_exclusive(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("ida_mcp.proxy._lock_path", lambda: lock_file)
        monkeypatch.setattr("ida_mcp.proxy._state_dir", lambda: tmp_path)
...
class TestVersionOk:
    def test_matching_versions(self):
        state = {"version": "2.0.0"}
        with patch("ida_mcp.proxy.get_version", return_value="2.0.0"):
    ...
    def test_current_unknown_passes(self):
        state = {"version": "1.0.0"}
        with patch("ida_mcp.proxy.get_version", return_value="unknown"):
            assert _version_ok(state) is True
...
class TestStopDaemon:
    ...
    def test_escalates_to_sigkill(self, monkeypatch):
        ...
        with (
            ...
            patch("ida_mcp.proxy._wait_for_exit", side_effect=[False, True]),
        ):
            _stop_daemon({"pid": 1234})
    ...
    def test_no_sigkill_on_windows(self, monkeypatch):
        ...
        with (
            patch("os.kill", side_effect=fake_kill),
            patch("ida_mcp.proxy._wait_for_exit", return_value=False),
        ):
            _stop_daemon({"pid": 1234})
        assert (1234, signal.SIGTERM) in kill_calls
...
class TestEnsureDaemon:
    def test_reuses_existing_daemon(self):
        ...
        with (
            patch("ida_mcp.proxy.read_state", return_value=state),
            patch("ida_mcp.proxy.daemon_alive", return_value=True),
            patch("ida_mcp.proxy._version_ok", return_value=True),
        ):
            result = _ensure_daemon()
        assert result is state

    def test_spawns_when_no_state(self, tmp_path, monkeypatch):
        monkeypatch.setattr("ida_mcp.proxy._lock_path", lambda: str(tmp_path / "daemon.lock"))
        monkeypatch.setattr("ida_mcp.proxy._state_dir", lambda: tmp_path)
        new_state = {"pid": 2, "host": "127.0.0.1", "port": 9090, "token": "new", "version": "v"}
        with (
            patch("ida_mcp.proxy.read_state", return_value=None),
            patch("ida_mcp.proxy._spawn_daemon", return_value=new_state) as mock_spawn,
        ):
            result = _ensure_daemon()
    ...
    def test_spawns_when_stale(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            patch("ida_mcp.proxy._version_ok", return_value=True),
            patch("ida_mcp.proxy.remove_state") as mock_remove,
            patch("ida_mcp.proxy._spawn_daemon", return_value=new_state),
        ):
            result = _ensure_daemon()
        assert result is new_state
    ...
    def test_double_check_under_lock(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            patch("ida_mcp.proxy.daemon_alive", return_value=True),
            patch("ida_mcp.proxy._version_ok", return_value=True),
            patch("ida_mcp.proxy._spawn_daemon") as mock_spawn,
        ):
            result = _ensure_daemon()
        assert result is state
    ...
    def test_version_mismatch_restarts_daemon(self, tmp_path, monkeypatch):
        ...
        with (
            patch("ida_mcp.proxy.read_state", return_value=old_state),
            patch("ida_mcp.proxy.daemon_alive", return_value=True),
            patch("ida_mcp.proxy._version_ok", return_value=False),
            patch("ida_mcp.proxy._stop_daemon") as mock_stop,
            patch("ida_mcp.proxy.remove_state") as mock_remove,
            patch("ida_mcp.proxy._spawn_daemon", return_value=new_state) as mock_spawn,
        ):
            result = _ensure_daemon()
...
class TestSpawnDaemon:
    ...
    def test_timeout_raises(self, monkeypatch):
        mock_proc = MagicMock()
        mock_proc.poll.return_value = None
        monkeypatch.setattr("ida_mcp.proxy._DAEMON_STARTUP_TIMEOUT", 0.01)
    ...
    def test_stderr_captured_to_log_file(self, tmp_path, monkeypatch):
        ...
        with (
            patch("ida_mcp.proxy.resolve_log_file", return_value=stderr_path),
            patch("subprocess.Popen", return_value=mock_proc),
            patch("ida_mcp.proxy.read_state", return_value=None),
...
class TestStop:
    def test_no_state_returns_false(self):
        with patch("ida_mcp.proxy.read_state", return_value=None):
