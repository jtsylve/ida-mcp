...
| `transforms.py` | `IDAToolTransform(CatalogTransform)` — pins common tools, adds `search_tools` and `execute` meta-tools, hides the rest from listing while keeping them callable by name |
