...
class _IdaCallFinder(ast.NodeVisitor):
    ...
    def visit_Await(self, node: ast.Await) -> None:
        # await call_ida(...) / await async_paginate_iter(...)
        # All arguments (including generators) are dispatched to main thread.
        if (
            isinstance(node.value, ast.Call)
            and isinstance(node.value.func, ast.Name)
            and node.value.func.id in _DISPATCH_FUNCTIONS
        ):
            self._mark_safe(node.value)
            return
    ...
    def _mark_safe(self, call_node: ast.Call) -> None:
        """Mark all sub-nodes of a dispatch call as safe."""
