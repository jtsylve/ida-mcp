...
- `detach_all(session_id, save=True)` detaches a session from all workers, terminating any whose session set becomes empty; falls back to `shutdown_all()` when session ID is `None`.
...
Workers track which MCP sessions are using them via `attach(session_id)` / `detach(session_id)` / `is_attached(session_id)` / `session_count`. Sessions are attached implicitly when a tool is called (via `RoutingTool.run()`) and explicitly on `open_database`. `close_database` detaches the session first, then terminates the worker only if no sessions remain (or `force=True`). When other sessions are still using the database, `close_database` returns a `detached` status instead of terminating. `save_database` and `close_database` check attachment before proceeding (unless `force=True`). A `_session_id(ctx)` helper in the supervisor extracts the session ID from the FastMCP `Context`.
